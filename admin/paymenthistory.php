<?php
 ...
 header('location:index2.php');
    ...
    $_SESSION['sidebarname']= 'Payment History';
    $sitio=getsitio($dbh);
    ...
    if ($_SERVER["REQUEST_METHOD"] == "POST" && isset($_POST['update'])) {
      ...
      if($_SERVER["REQUEST_METHOD"] == "POST" && isset($_POST['delete'])){
        ...
        $delete1="DELETE FROM payments WHERE bill_id=:id";
        $deletebill1=$dbh->prepare($delete1);
        $deletebill1->bindParam(':id',$id,PDO::PARAM_STR);
        try {
        $deletebill1->execute();
          $deletebill->execute();
        ...
        <label for="sitio" class="col-form-label pr-2">Sitio:</label>
            <select id="sitio" name="sitio" class="form-select form-select-sm" required="required" >
              ...
              function bill($dbh, $filterDate) {
                ...
                sitio.sitio_name,
                upper(bill.payment) AS paymentstatus,
                bill.id as bill_id,
         ...
         LEFT JOIN payments ON payments.bill_id = bill.id 
         LEFT JOIN employee ON bill.encoder_id = employee.user_id 
         WHERE DATE_FORMAT(bill.reading_date, '%Y-%m') = :currentDate AND bill.payment = 'paid'
         GROUP BY consumers.serial_no";
            ...
            $results = bill($dbh, $filterDate);
                   ...
                   foreach($results as $list){ 
                    ...
                    $datepaid = new Datetime($list->date_paid);
                    $paid = $datepaid ->format('F j , Y');
                    ?>
                        <tr>
                        <td><?php echo htmlentities($list->serial_no); ?></td>
                       <td><?php echo htmlentities($list->readername ?? "Encoder"); ?></td>
                          <td><?php echo htmlentities($list->fullname) ?></td>
                            ...
                            <th><?php echo $paid?></th>
                            <td>  <p class="<?php echo ($list->paymentstatus == "PAID") ? "text-success" : "text-warning"; ?>">
        <?php echo htmlentities($list->paymentstatus); ?>
    </p></td>
                            <td>
                                <!-- <button type="button" class="btn text-primary fs-4 border-0 m-0 p-1 updateModalBtn" id="updateModalBtn" title="Update" data-bs-toggle="modal" data-bs-target="#updateModal" data-serial="<?php echo $list->id ?>" data-consumer="<?php echo $list->id ?>" data-bill="<?php echo $list->bill_id ?>"><i class='bx bx-calendar-edit'></i></button> -->
