...
$query = "
    SELECT sitio.id, sitio.sitio_name, COALESCE(SUM(bill.consumption), 0) AS total_consumption
    FROM sitio
    LEFT JOIN consumers ON consumers.sitio = sitio.id
    LEFT JOIN bill ON bill.serial_number = consumers.serial_no 
                    AND bill.status = 'billed' 
                    AND MONTH(bill.reading_date) = :month
    GROUP BY sitio.id
    ORDER BY sitio.id";
...
foreach ($results as $row) {
    $dataPoints[] = [
        "label" => $row['sitio_name'], 
        "y" => (int)$row['total_consumption']
    ];
}
...
function fetchConsumptionData($dbh,$year) {
    $query = "
        SELECT MONTH(reading_date) AS month, SUM(consumption) AS total_consumption
        FROM bill
        WHERE status = 'billed' AND YEAR(reading_date) = :year 
...
$data_prev_year = fetchConsumptionData($dbh,  $previous_year);
$data_current_year = fetchConsumptionData($dbh,  $current_year);
...
?>
      ...
      <div class=" col-sm-7 bg-white  rounded p-3 shadow-sm">
        <p class="pb-2">Sitio's Current Month Consumption(per cubic metre)</p>
  ...
  <div class="row gy-4 p-0">
    ...
    <div class="col-sm-4 shadow-sm  p-0 bg-white" id="queries">
         <div class=" mt-2 p-0 d-flex align-items-start">
            <p class="ps-3 mb-auto p-1">Messages</p>
    </div>
<div class="chats">
<?php $sql ="SELECT * FROM messages WHERE status= ''";
...
?>
        ...
        <div class="row m-2 border rounded mb-1">
         <div class="col-10 p-1 border-end ">
            ...
            <p class="mb-0 p-1 "><?php echo htmlentities($result->Fullname); ?></p>
            <p class="mb-0 p-1 date"><?php echo htmlentities($result->date); ?></p>
            </div>
            <p class="mb-0 text-start p-1 truncated-text"><?php echo htmlentities($result->message); ?></p>
         </div>
         <div class="col-2 p-0">
         <button type="button" class="border h-100 w-100 d-flex justify-content-center align-items-center" title="Reply Message" data-bs-toggle="modal" data-bs-target="#replymodal-<?php echo htmlentities($result->id); ?>">
...
<?php 
    ...
    $number2 =2;
    $number3 =3;
    $number = intval($number1 . $number2 . $number3);
?>
...
<div class="container mt-3 rounded  shadow-sm bg-white p-2">
            ...
            <form action="<?=$_SERVER['PHP_SELF']?>" method="POST" id="filterForm">
             ...
             $sql = "SELECT bill.*, 
                 ...
                 WHERE consumers.warning = 'active' OR consumers.warning = 'pending' AND bill.due_date = :currentDate 
